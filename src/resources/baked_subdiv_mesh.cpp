...
void BakedSubdivMesh::_update_subdiv() {
	if (data_mesh.is_valid()) {
		...
		for (int surface_index = 0; surface_index < data_mesh->get_surface_count(); surface_index++) {
			const Array &triangle_arrays = baker.get_baked_arrays(data_mesh->surface_get_arrays(surface_index),
					subdiv_level, data_mesh->surface_get_format(surface_index), data_mesh->surface_get_topology_type(surface_index));
			add_surface_from_arrays(Mesh::PRIMITIVE_TRIANGLES, triangle_arrays, Array(), Dictionary(), 0);
